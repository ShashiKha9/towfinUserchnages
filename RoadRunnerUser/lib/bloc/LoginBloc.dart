...
class LoginBloc {
  ...
  loginReq(String emailId, String password, String deviceId,
    ...
    bloc.userProfileReq(
        await _getId(), context, response.access_token, response.token_type);
  ...
  userProfileReq(String deviceId, BuildContext context, String accessToken,
      String tokenType) async {
    UserProfileRespo response = await _repository.getUserProfile(
        deviceId, context, accessToken, tokenType);
    SharedPreferences prefs = await SharedPreferences.getInstance();
    ...
    prefs.setString(SharedPrefsKeys.TOKEN_TYPE, tokenType);
  ...
  fetchAllproviderListReq(
    ...
    ProvidersListResponse response = await _repository.fetchAllProviersReq(
      ...
      if(response.allServicesList.length==0){
        showToast("Providers not found near you, please try after some time.", context);
      }
